...
TARGETS = {
    "swiggy": "https://careers.swiggy.com/list.html?dept=Engineering&loc=1",
    "lowes_india": "https://talent.lowes.com/in/en/search-results",
    "makemytrip": "https://careers.makemytrip.com/prod/careerPlaybook/software-engineering",
...
INTERESTING = re.compile(
    r"(?:job|jobs|career|search|opening|position|vacan|requisition|opportun|api|graphql|query)",
    re.I,
)
    ...
    with sync_playwright() as p:
        ...
        def on_request(req):
            ...
            evidence["interesting_requests"].append({
                ...
                "url": req.url,
                "post_data": _clip(req.post_data or "", 5000),
            })
        ...
        def on_response(resp):
            ...
            if url_ in seen_response_urls:
                return
            if "json" not in ct and not INTERESTING.search(url_):
                return
            ...
            try:
                if "json" in ct:
                    ...
                    row["sample"] = _clip(json.dumps(payload, ensure_ascii=False), 8000)
                else:
                    text = resp.text()
                    row["sample"] = _clip(text, 3000)
            except Exception as exc:  # diagnostics must never hide the URL
        ...
        # Give client apps time to boot and make their search requests. Scroll and
        # click common controls so lazy-loaded/paginated endpoints become visible.
        for _ in range(6):
            try:
                ...
                break
        for label in ("Load more", "Show more", "View more", "More jobs", "Next"):
            try:
        ...
        try:
            ...
            evidence["body_text"] = _clip(page.locator("body").inner_text(timeout=3000), 12000)
            ...
            evidence["resource_urls"] = [u for u in resources if INTERESTING.search(u)][:500]
...
def main() -> int:
    ...
    for row in result["responses"][:30]:
        print(f"RESPONSE {row['status']} {row['content_type']} {row['url']}")
        if row.get("json_shape") is not None:
            print("SHAPE", json.dumps(row["json_shape"], ensure_ascii=False)[:2500])
    for row in result["interesting_requests"][:50]:
        print(f"REQUEST {row['method']} {row['resource_type']} {row['url']}")
        if row.get("post_data"):
            print("POST", row["post_data"][:1200])
    return 0
